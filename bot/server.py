...
def default_command(message):
    ...
    checker_result = " not " if check_text(message_text) else ""
    result_text = "Received text message from " + message.from_user.first_name + \
                  "(@" + message.from_user.username + \
                  "): [" + message_text + \
                  "]. This text is " + checker_result + "valid. Thank you!"
    bot.send_message(message.chat.id, result_text)
...
def receive_img(message):
    ...
    if check_text_result:
        ...
        result_text = "Received message with adult content from user: " + message.from_user.first_name + \
                  "(@" + message.from_user.username + \
                  "): [" + invalid_text_segment + "]. We notified admin's of the current chat."
        bot.send_message(message.chat.id, result_text)
